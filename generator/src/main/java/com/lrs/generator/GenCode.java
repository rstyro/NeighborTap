...
public class GenCode {
     ...
     * 数据库链接相关信息
     */
    public static final String DB_URL = "jdbc:mysql://127.0.0.1:3306/neighbor?useUnicode=true&characterEncoding=UTF-8&allowMultiQueries=true&serverTimezone=GMT%2B8";
...
//                .templateConfig(config->config.entity("/templates/entity1.java"))
        ...
        if("1".equalsIgnoreCase(isGeneratorPage)){
            // 自定义生成代码的模板
            fastAutoGenerator.templateConfig(config->config.entity("/templates/codeTemplates/entity.java"));
            fastAutoGenerator.templateConfig(config->config.service("/templates/codeTemplates/Service.java"));
            fastAutoGenerator.templateConfig(config->config.serviceImpl("/templates/codeTemplates/ServiceImpl.java"));
            fastAutoGenerator.templateConfig(config->config.controller("/templates/codeTemplates/Controller.java"));
            // 自定义页面生成
            fastAutoGenerator.injectionConfig(consumer->{
                ...
                customFiles.add(new CustomFile.Builder()
                        ...
                        .fileName("_list.html")
                        .templatePath("/templates/pageTemplates/list.ftl").enableFileOverride().build());
